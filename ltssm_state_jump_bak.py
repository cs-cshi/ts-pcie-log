...
def extract_ltssm_info(log_path, start_str, end_str):
    ...
    with open(log_path, 'r', errors='ignore') as file:
        ...
        for line in log_lines:
            ...
            if start_flag:
                ...
                elif "eye eight phase23 value" in line:
    ...
    for sub_ltssm_group, sub_0x20560, sub_ltssm_st_value, sub_histogram_data, sub_histogram_data2_height, sub_eq_obj_value, sub_eq_obj_tap4_value, sub_eq_obj_tap5_value, sub_eq_obj_tap6_value, sub_eq_obj_ffe_taps_value, sub_eye_height_value, sub_adc_os_cal_dig_cor_gen_value, sub_adc_os_cal_actl_gen_value, sub_take_it_out_value, sub_take_it_out_early_value,sub_eye_height_phase23_value,sub_eye_height_l0_value in itertools.zip_longest(
        ...
        for item in current_eye_height_phase23_value:
            sheet.cell(row=excel_row, column=excel_column, value=item)
            excel_row += 1
...
def test():
    ...
    # directory_path = r'D:\zygfile\desktoop\S5000c-80\问题排查及数据\reg-print\20240904\#67'
    # directory_path = r'C:\Users\46439\Documents\20240910\pcie'
...
def merge_xls_files(directory, output_file):
    ...
    for filename in os.listdir(directory):
        if filename.endswith('.xls'):
            ...
            # 使用pandas读取xls文件，这里假设每个文件的结构相同
            # 注意：如果xls文件包含多个工作表，你需要指定sheet_name参数
            df = pd.read_excel(file_path, engine='xlrd')
            # 将读取的数据添加到merged_data中
            merged_data = pd.concat([merged_data, df], ignore_index=True)

            # 将合并后的数据写入新的xls文件
    # 注意：pandas的ExcelWriter默认使用xlsxwriter作为引擎，但我们可以指定xlwt用于xls
...
output_xls_file = r'D:\zygfile\desktoop\S5000c-80\问题排查及数据\reg-print\0830-fw12-log/merged_output.xls'
output_log_path = input_log_path.split(".log")[0] + '_preprocessed.log'

# 调用函数
merge_xls_files(directory_path, output_xls_file)
'''
test()
